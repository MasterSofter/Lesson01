...
public class GameViewer : MonoBehaviour, IGameViewer
{
    ...
    [SerializeField] private GameObject _fruitPrefub;
    ...
    ******************************************************************/
    ...
    public void SpawnBomb(BombDm bombDm) {
        ...
        GameObject newGameObj = Instantiate(_fruitPrefub, _spawnPoints[rundomIndexSpawnPoint].position, this.transform.rotation);
        newGameObj.GetComponent<Rigidbody2D>().AddForce(_spawnPoints[rundomIndexSpawnPoint].transform.up * _startSpawnFruitForce, ForceMode2D.Impulse);
        newGameObj.GetComponent<Rigidbody2D>().AddTorque(_startFruitTorgue);
    ...
    public IEnumerator ShowRestartGame()
    {
        ...
        if (GameDataModel.GameState == EnumGameState.GameOver)
        {
    ...
    public void Init(IEventBus eventBus,GameDataModel gameDataModel)
    {
        _eventBus = eventBus;
